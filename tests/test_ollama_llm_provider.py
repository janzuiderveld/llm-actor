...
def test_parse_llm_model_spec():
    ...
    assert parse_llm_model_spec("anthropic-claude-3-5-sonnet-20241022") == (
        "anthropic",
        "claude-3-5-sonnet-20241022",
    )
    assert parse_llm_model_spec("claude-3-5-sonnet-20241022") == (
        "anthropic",
        "claude-3-5-sonnet-20241022",
    )
...
def test_build_llm_service_ollama():
    ...
    service, provider = build_llm_service(
        config,
        google_api_key=None,
...
def test_build_llm_service_anthropic():
    config = RuntimeConfig()
    config.llm.model = "anthropic-claude-3-5-sonnet-20241022"
    ...
    assert service.__class__.__name__ == "AnthropicLLMService"


def test_normalize_model_for_service():
    config = RuntimeConfig()
    config.llm.model = "ollama-gemma3:4b"
    ollama_service, _ = build_llm_service(
        config,
        google_api_key=None,
        anthropic_api_key=None,
        openai_api_key=None,
    ...
    config.llm.model = "openai-gpt-5.2-chat-latest"
    openai_service, _ = build_llm_service(
        config,
        google_api_key=None,
        anthropic_api_key=None,
        openai_api_key="test-key",
    ...
    config.llm.model = "anthropic-claude-3-5-sonnet-20241022"
    anthropic_service, _ = build_llm_service(
        config,
        google_api_key=None,
        anthropic_api_key="test-key",
        openai_api_key=None,
    ...
    assert (
        normalize_model_for_service("anthropic-claude-3-5-sonnet-20241022", anthropic_service)
        == "claude-3-5-sonnet-20241022"
    )

    config.llm.model = "gemini-2.5-flash"
    google_service, _ = build_llm_service(
        config,
        google_api_key="test-key",
        anthropic_api_key=None,
        openai_api_key=None,
